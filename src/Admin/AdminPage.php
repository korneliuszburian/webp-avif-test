...
class AdminPage {
    ...
    public function registerHooks(): void {
        ...
        // Ajax handlers
        add_action('wp_ajax_wp_image_optimizer_get_progress', [$this, 'ajaxGetProgress']);
        add_action('wp_ajax_wp_image_optimizer_convert_single', [$this, 'ajaxConvertSingle']);
        add_action('wp_ajax_wp_image_optimizer_bulk_convert', [$this, 'ajaxBulkConvert']);
    }
    ...
    public function ajaxGetProgress(): void {
        ...
        if (!$process_id) {
            wp_send_json_error(['message' => 'Missing process ID']);
        }
        
        $progress = $this->progressManager->getProgress($process_id);
        if (!$progress) {
            wp_send_json_error(['message' => 'Process not found']);
        }
    ...
    public function ajaxConvertSingle(): void {
        ...
        if ($result['success']) {
            ...
            wp_send_json_error(['message' => __('Failed to convert image', 'wp-image-optimizer')]);
        }
    }
